...
class FileListView(TemplateView):
    ...
    def get_breadcrumbs(self, folder):
        ...
        while current:
            breadcrumbs.insert(0, {"name": current.name, "id": current.id})
            current = current.parent

        # Agregar la raíz al principio
        breadcrumbs.insert(0, {"name": "Inicio", "id": None})
...
def stream_file(request, id):
    ...
    response["Content-Disposition"] = (
        f'inline; filename="{quote(os.path.basename(file_path))}"'
    )
    return response
...
def download_folder(request, id):
    ...
    if os.path.exists(zip_path) and (time.time() - os.path.getmtime(zip_path)) < 3600:
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{quote(folder.name)}.zip"'
        )
        return response
    ...
    try:
        ...
        # Servir el archivo
        response = FileResponse(
            open(tmp_zip_path, "rb"), content_type="application/zip"
        )
        response["Content-Disposition"] = (
            f'attachment; filename="{quote(folder.name)}.zip"'
        )
...
def __recursiveAddFileToDb(file_data, server, parent=None):
    ...
    existing_file = File.objects.filter(
        path=file_data.get("path"), server=server
    ).first()
